...
import os
import random
from arcade.color import NADESHIKO_PINK, SCARLET
import time
import csv
...
class Game_Over_View(arcade.View):
    ...
    def __init__(self, score, player_name, leaderboard):
        """ this is run once when we switch to this view """
        super().__init__()
        self.score = score
    ...
    def on_draw(self):
        ...
        for index, row in enumerate(self.leaderboard):
            arcade.draw_text(f'{row[0]}      {row[1]}', SCREEN_WIDTH/2 + 30, SCREEN_HEIGHT/2 + 50 - 20*index, arcade.color.WHITE, font_size=15, anchor_x="center")


        # Draws the Game over text with the player score on it
        arcade.draw_text(f"Game Over / Score: {self.score} / Press Enter to play again", SCREEN_WIDTH / 2, SCREEN_HEIGHT / 2-75, arcade.color.WHITE, font_size=20, anchor_x="center")
...
class MyGame(arcade.View):
    ...
    def setup(self):
        ...
        # extra player sprite for development purposes
        # self.player_sprite = Player(image_source, CHARACTER_SCALING, 5, PLAYER_SPEED, Player_Laser_Basic())
    ...
    def check_game_over(self):
        if self.player_sprite.health <= 0:
            ...
            with open('leaderboard.csv', 'r', newline='') as csvfile:
                ...
                # prints leaderboard as a list
                data = list(leaderboard)
                place = 5
                
                # calculates place in the leaderboard
                for row in data:
                    if self.score >= int(row[1]):
                        place -= 1
                
                # updates leaderboard as a list
                data.insert(place, [self.player_name,self.score])
                data = data[:5]

            # with updated data, writes back to the leaderboard csv file
            with open('leaderboard.csv', 'w', newline='') as csvfile:
                new_leaderboard = csv.writer(csvfile, delimiter=',')

                for row in data:
                    new_leaderboard.writerow(row)

            # starts the game over view
            game_over_view = Game_Over_View(self.score, self.player_name, data)
